...
awk '/let sumA = \{|__TISH_GF_sumA\.with\(/{f=1} f&&/sumAHere|sumAMasked|sumB/{f=0} f' "$G" | grep -q 'A.borrow()'
check $? "A — never passed to a native — reads through the typed Vec path"
awk '/let sumB = \{|__TISH_GF_sumB\.with\(/{f=1} f&&/sumAHere|sumAMasked|let sumA |__TISH_GF_sumA\.with/{f=0} f' "$G" | grep -q 'get_index(&tishlang_runtime::vm_read(&B)'
...
awk '/let sumA = \{|__TISH_GF_sumA\.with\(/{f=1} f&&/sumAHere|sumAMasked|sumB/{f=0} f' "$G" | grep -q 'get_index' && { echo "FAIL A is boxed too"; fail=1; } || echo "ok   A is NOT boxed (the difference is real, not universal)"
