#!/usr/bin/env bash
# rts-fog verify — the acceptance test for the flow field: it must stay inside the frame budget
...
LOG=/tmp/rts-fog-verify.log
GBA_SHOT_LOG=1 "$ROOT/scripts/screenshot.sh" rts-fog.gba /tmp/rts-fog-verify.png 1500 \
  >"$LOG" 2>&1 || true

check "no panic in a 1500-frame run" bash -c '! grep -qiE "panic|Bad memory|not implemented" "$1"' _ "$LOG"

# The fog state machine: `S` samples (2,2) then (27,17). `S12` is the scout having explored its
# start corner (1) and currently seeing the far one (2) — i.e. reveal happened, explored persisted,
# and the two states are distinct. A run that never leaves S00 has fog that does nothing.
check "fog reveals, and explored persists behind the scout" bash -c 'grep -qE "S12" "$1"' _ "$LOG"

# Dirty-cell tracking: a settled camera must write NOTHING. If this fails the shroud is repainting
# the whole window every frame and the cost scales with map size rather than with movement.
check "a settled camera writes zero shroud cells" bash -c 'grep -qE " W0 " "$1"' _ "$LOG"
...
if [[ $FAILED -eq 0 ]]; then echo "rts-fog verify ok"; else echo "rts-fog verify FAILED"; fi
exit $FAILED
